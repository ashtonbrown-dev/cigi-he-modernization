...
struct _CIGI_PACKET_HEADER {
    unsigned short packet_size;      /** The size of the packet, in bytes. */
    unsigned short packet_id;        /** Packet ID number (type). */
...
// See documentation for non-templated CigiAddFixedPacket in cigi4.cpp.
...
int CigiAddFixedPacket(int sessionid,
                       ...
                       const char *replaceMask = NULL)
{
    const uint16_t packetSize = (uint16_t)sizeof(PacketT);
...
int CigiAddPacketReplaceSymbolTexturedCircleDef(const int session, const CIGI_SYMBOL_TEXTURED_CIRCLE_DEF *packet);
int CigiAddPacketReplaceSymbolTexturedPolygonDef(const int session, const CIGI_SYMBOL_TEXTURED_POLYGON_DEF *packet);
