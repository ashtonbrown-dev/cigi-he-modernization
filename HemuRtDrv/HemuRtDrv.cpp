...
#define PRIORITY_NORMAL         THREAD_PRIOTIRY_NORMAL
...
// Priority level codes
#define PRIORITY_HIGHEST        (RT_PRIORITY_MIN + 4)
#define PRIORITY_HIGHER         (RT_PRIORITY_MIN + 3)
#define PRIORITY_HIGH           (RT_PRIORITY_MIN + 2)
#define PRIORITY_NORMAL         (RT_PRIORITY_MIN + 1)
#define PRIORITY_LOW            (RT_PRIORITY_MIN)
#endif
...
ULONG RTFCNDCL WaitForWin32ProcToDie(void *nContext);
ULONG RTFCNDCL SendRcvThread(void *nContext);
ULONG RTFCNDCL ShutdownThread(void *nContext);
ULONG RTFCNDCL RecordThread(void *nContext);
ULONG RTFCNDCL PlaybackThread(void *nContext);
...
RTXSharedBufferQ RecPlaybackBufferQueue;
...
void _cdecl main(int  argc, char **argv, char **envp)
{
    ...
    // If we are building the non-realtime Windows version, set future
    // calls to RtSleepFt() to truncate instead of round if we have
    // multiple processors.  If this is the RTX version, this call has no
    // effect.
    if ((argc > 2) && (((strcmp(argv[1], "-mp") == 0)) || (strcmp(argv[2], "-mp") == 0))) {
        mp = 1;
        LARGE_INTEGER useless;
        RtGetClockResolution(CLOCK_FASTEST, &useless);
    ...
    strcpy(msg.text, "Real-time driver loaded.");
    SendGuiMessage(&msg);

    RtWaitForSingleObject(hShutdownThread, INFINITE);

    if (verbose)
        RtPrintf("Shutdown complete.\n");
...
int Startup(void)
{
    ...
    // If we are building the non-realtime Windows version, set the
    // internal high-performance counter frequency variable for future calls
    // to RtGetClockTime().  If this is the RTX version, it has no effect.
    LARGE_INTEGER useless;
    RtGetClockTimerPeriod(CLOCK_FASTEST, &useless);
    ...
    if (WSAStartup(MAKEWORD(1, 1), &wsainfo) != 0) {
        if (verbose)
            RtPrintf("Winsock not available.\n");
        return 0;
    }
...
int InitializeShutdown(void)
{
    ...
    hWin32ProcDiedMutex = RtCreateMutex(NULL, FALSE, "HemuForceShutdownMutex");

    // Create the shutdown event.
    hShutdownEvent = RtCreateEvent(NULL, TRUE, FALSE, "HemuRTStartShutdownEvent");
    if (!hShutdownEvent) {
        if (verbose)
            RtPrintf("Cannot create shutdown event.  Error = %d\n", GetLastError());
        return 0;
    }

    // Set the priority of main thread.
    if (RtSetThreadPriority(GetCurrentThread(), PRIORITY_HIGH) == FALSE) {
        if (verbose)
            RtPrintf("Cannot set main thread priority.  Error = %d\n", GetLastError());
        return 0;
    }
    ...
    hWaitForWin32ProcToDieThread = RtCreateThread(0, 0, WaitForWin32ProcToDie, NULL, CREATE_SUSPENDED, 0);
    if (!hWaitForWin32ProcToDieThread) {
        if (verbose)
            RtPrintf("Cannot create thread.  Error = %d\n", GetLastError());
        return 0;
    }
    if (RtSetThreadPriority(hWaitForWin32ProcToDieThread, PRIORITY_HIGHEST) == FALSE) {
        if (verbose)
            RtPrintf("Cannot set thread priority.  Error = %d\n", GetLastError());
        return 0;
    }

    SuspendCount = RtResumeThread(hWaitForWin32ProcToDieThread);
    if (SuspendCount == 0xFFFFFFFF) {
        if (verbose)
            RtPrintf("Cannot start thread.  Error = %d\n", GetLastError());
        return 0;
    }

    // Create a high-priority shutdown thread.  When the Win32 process triggers the
    // StartShutdownEvent, the thread is free to run.
    hShutdownThread = RtCreateThread(0, 0, ShutdownThread, NULL, CREATE_SUSPENDED, 0);
    if (!hShutdownThread) {
        if (verbose)
            RtPrintf("Cannot create thread.  Error = %d\n", GetLastError());
        return 0;
    ...
    if (RtSetThreadPriority(hShutdownThread, PRIORITY_HIGHER) == FALSE) {
        if (verbose)
            RtPrintf("Cannot set shutdown thread priority.  Error = %d\n", GetLastError());
        return 0;
    }

    SuspendCount = RtResumeThread(hShutdownThread);
    if (SuspendCount == 0xFFFFFFFF) {
        if (verbose)
            RtPrintf("Cannot start shutdown thread.  Error = %d\n", GetLastError());
        return 0;
    }
...
int InitializeSendRcv(void)
{
    ...
    //hReceivedMessageEvent = RtCreateEvent(NULL, FALSE, FALSE, "HemuRTReceivedMessageEvent");
    ...
    hSendRcvThread = RtCreateThread(0, 0, SendRcvThread, NULL, CREATE_SUSPENDED, 0);
    if (!hSendRcvThread) {
        if (verbose)
            RtPrintf("Cannot create thread.  Error = %d\n", GetLastError());
        return 0;
    ...
    if (RtSetThreadPriority(hSendRcvThread, RT_PRIORITY_MAX) == FALSE) {
        if (verbose)
            RtPrintf("Cannot set send/receive thread priority.  Error = %d\n", GetLastError());
        return 0;
    }

    SuspendCount = RtResumeThread(hSendRcvThread);
    if (SuspendCount == 0xFFFFFFFF) {
        if (verbose)
            RtPrintf("Cannot start send/receive thread.  Error = %d\n", GetLastError());
        return 0;
    }
...
int InitializeSockets(MESSAGE_SET_ADDR *MsgSetAddr)
{
    ...
    if ((short)sndsock == INVALID_SOCKET) {
        if (verbose)
            RtPrintf("Cannot create socket for sending.  Error = %d\n", WSAGetLastError());

        RtSetEvent(hShutdownEvent);
    }
    if (verbose)
        RtPrintf("Socket %d opened for sending to port %d at %s.\n",
    ...
    if ((short)hudsock == INVALID_SOCKET) {
        if (verbose)
            RtPrintf("Cannot create socket for sending HUD data.  Error = %d\n", WSAGetLastError());

        RtSetEvent(hShutdownEvent);
    }
    if (verbose)
        RtPrintf("Socket %d opened for sending HUD data to port %d at %s.\n",
    ...
    if ((short)rcvsock == INVALID_SOCKET) {
        if (verbose)
            RtPrintf("Cannot create socket for receiving.  Error = %d\n", WSAGetLastError());
    ...
    if (setsockopt(rcvsock, SOL_SOCKET, SO_REUSEADDR, (const char *)&sockparam, sizeof(int))
        == SOCKET_ERROR) {
        if (verbose)
            RtPrintf("Cannot make address reusable.\n");
    ...
    if (ioctlsocket(rcvsock, FIONBIO, &arg) == SOCKET_ERROR) {
        if (verbose)
            RtPrintf("Cannot make receive socket non-blocking.  Error = %d\n", WSAGetLastError());
    ...
    if (bind(rcvsock, (SOCKADDR *)&raddr, sizeof(SOCKADDR)) == SOCKET_ERROR) {
        if (verbose)
            RtPrintf("Cannot perform bind on receive socket.  Error = %d\n", WSAGetLastError());

        RtSetEvent(hShutdownEvent);
    }
    if (verbose)
        RtPrintf("Socket %d bound to port %d at local address.\n",
...
ULONG RTFCNDCL WaitForWin32ProcToDie(void *nContext)
{
    if (hWin32ProcDiedMutex) {
        ...
        RtWaitForMultipleObjects(2, handles, FALSE, INFINITE);
    }
...
ULONG RTFCNDCL SendRcvThread(void *nContext)
{
    ...
    do {
        ...
        if (buffsize > 0) {
            ...
            RtGetClockTime(CLOCK_FASTEST, &t_rcv);
            ...
            if (OperateMode == MODE_PLAYBACK) {
                ...
                if (readsize) {
                    ...
                    RtGetClockTime(CLOCK_FASTEST, &t_send);
            ...
            RtSleepFt(&sleep_interval);

#ifdef NO_RTX
            // For multiprocessor machines, the RtSleepFt() call
            ...
            if (i++ < icount)
                ...
                // Send a status message.  This will happen 4 times a second if we
                // are not connected to the IG.
                SendGuiMessage(&MsgNoConnect);
            }
#endif
        }

        RtGetClockTime(CLOCK_FASTEST, &t1);
        if (RunState && (OperateMode == MODE_NORMAL)) {
            UpdateEntities(dt.QuadPart / 10000000.0f);
        }
        dt.QuadPart = t1.QuadPart - t2.QuadPart;
        RtGetClockTime(CLOCK_FASTEST, &t2);
    ...
    } while (RtWaitForSingleObject(hShutdownEvent, 0) != WAIT_OBJECT_0);
...
ULONG RTFCNDCL ShutdownThread(void *nContext)
{
    ...
    RtWaitForMultipleObjects(3, handles, FALSE, INFINITE);

    if (verbose)
        RtPrintf("Received shutdown event or Win32 process has died.\n");
...
ULONG RTFCNDCL RecordThread(void *nContext)
{
    ...
    if (verbose && !hRecordFile) {
        RtPrintf("Record error.  Invalid file handle.\n");
        return(0);
    }

    if (verbose)
        RtPrintf("Record thread created.\n");

    // For some reason this event isn't being automatically reset after
    // it is released and re-created.  Reset it here.
    RtResetEvent(hRecordShutdownEvent);

    HANDLE handles[2] = { hRecordShutdownEvent, hShutdownEvent };
    while ((RtWaitForMultipleObjects(2, handles, FALSE, 1) == WAIT_TIMEOUT)
    ...
    if (verbose)
        RtPrintf("RecordThread terminated.\n");
...
int StartRecord(char *filename)
{
    ...
    if (hRecordFile == INVALID_HANDLE_VALUE) {
        ...
        RtPrintf("Cannot create record file %s.  Error code = %d\n", filename, GetLastError());
        hRecordFile = NULL;
        return 0;
    } else if (verbose)
        RtPrintf("File %s opened for recording.\n", filename);

    Recording = TRUE;

    // Create low-priority record thread.
    hRecordShutdownEvent = RtCreateEvent(NULL, TRUE, FALSE, "HemuRTRecordShutdownEvent");
    hRecordThread = CreateThread(0, 0, RecordThread, NULL, CREATE_SUSPENDED, 0);
    RtSetThreadPriority(hRecordThread, PRIORITY_LOW);
    RtResumeThread(hRecordThread);
...
void StopRecord(void)
{
    ...
    RtSetEvent(hRecordShutdownEvent);
    RtWaitForSingleObject(hRecordThread, INFINITE);

    CloseHandle(hRecordFile);
    RtCloseHandle(hRecordShutdownEvent);
    hRecordFile = NULL;
...
ULONG RTFCNDCL PlaybackThread(void *nContext)
{
    ...
    if (verbose && !hPlaybackFile) {
        RtPrintf("Read error.  Invalid file handle.\n");
        return(0);
    }

    if (verbose)
        RtPrintf("Read thread created.\n");

    // For some reason this event isn't being automatically reset after
    // it is released and re-created.  Reset it here.
    RtResetEvent(hPlaybackShutdownEvent);
    ...
    RtCloseHandle(hPlaybackShutdownEvent);
    hPlaybackFile = NULL;
    ...
    if (verbose)
        RtPrintf("PlaybackThread terminated.\n");
...
int StartPlayback(char *filename)
{
    if (hRecordFile) {
        if (verbose)
            RtPrintf("Recording. Attempting to stop for playback.\n");
    ...
    if (hPlaybackFile) {
        if (verbose)
            RtPrintf("Previous playback has not stopped. Attempting to stop.\n");
    ...
    if (hPlaybackFile == INVALID_HANDLE_VALUE) {
        ...
        RtPrintf("Cannot open playback file %s.  Error code = %d\n", filename, GetLastError());
        hPlaybackFile = NULL;
        return 0;
    } else if (verbose)
        RtPrintf("File %s opened for playback.\n", filename);

    // Create low-priority read thread. We can reuse the record shutdown event.
    hPlaybackShutdownEvent = RtCreateEvent(NULL, FALSE, FALSE, "HemuRTPlaybackShutdownEvent");
    hPlaybackThread = CreateThread(0, 0, PlaybackThread, NULL, CREATE_SUSPENDED, 0);
    RtSetThreadPriority(hPlaybackThread, PRIORITY_LOW);
    RtResumeThread(hPlaybackThread);
...
void StopPlayback(void)
{
    ...
    // Allow the record thread to stop.
    RtSetEvent(hPlaybackShutdownEvent);
    RtWaitForSingleObject(hPlaybackThread, INFINITE);

    if (verbose)
        RtPrintf("Playback thread has been stopped successfully.\n");
}
...
void CheckMessages(void)
{
    ...
    while ((msg = RcvGuiMessage()) != NULL) {
        ...
        case MSG_ADD_ENTITY: {
            if (verbose)
                RtPrintf("Received MESSAGE_ADD_ENTITY from GUI process.  ");
            ...
            if (entity) {
                ...
                if (verbose) {
                    ...
                    if (!entity->GetAndLockCigiData(&entity_state)) {
                        ...
                        if (verbose)
                            RtPrintf("     Entity %d is locked.  Queued for retry.\n", entity_state.id);

                        break;
                    } else {
                        entity->Unlock();
                    }
                    RtPrintf("     Entity ID = %d.\n", entity_state.id);
                }

                EntityList.InsertTail(entity);
            } else {
                RtPrintf("Out of memory!!!  Could not create entity.\n");
            }
        ...
        case MSG_DEL_ENTITY: {
            if (verbose)
                RtPrintf("Received MESSAGE_DEL_ENTITY from GUI process (ID = %d).\n",
            ...
            if (!shared_obj->GetAndLockCigiData(&data)) {
                ...
                if (verbose)
                    RtPrintf("     Entity is locked. Queued for retry.\n");
        ...
        case MSG_CLEAR_ENTITIES:
            if (verbose)
                RtPrintf("Received MESSAGE_CLEAR_ENTITIES from GUI process.\n");
        ...
        case MSG_ADD_VIEW: {
            if (verbose)
                RtPrintf("Received MESSAGE_ADD_VIEW from GUI process.  ");
            ...
            if (!view->GetAndLockViewDef(&viewdef)) {
                ...
                if (verbose)
                    RtPrintf("     View %d is locked. Queued for retry.\n", viewdef.view_id);
        ...
        case MSG_DEL_VIEW: {
            if (verbose)
                RtPrintf("Received MESSAGE_DEL_VIEW from GUI process (ID = %d).\n",
        ...
        case MSG_CLEAR_VIEWS:
            if (verbose)
                RtPrintf("Received MESSAGE_CLEAR_VIEWS from GUI process.\n");
        ...
        case MSG_ADD_VIEWGROUP: {
            if (verbose)
                RtPrintf("Received MESSAGE_ADD_VIEWGROUP from GUI process.  ");
            ...
            if (!group->GetAndLockCigi(&group_data)) {
                ...
                if (verbose)
                    RtPrintf("     View group %d is locked. Queued for retry.\n", group_data.group_id);
        ...
        case MSG_DEL_VIEWGROUP: {
            if (verbose)
                RtPrintf("Received MESSAGE_DEL_VIEWGROUP from GUI process (ID = %d).\n",
        ...
        case MSG_CLEAR_VIEWGROUPS:
            if (verbose)
                RtPrintf("Received MESSAGE_CLEAR_VIEWGROUPS from GUI process.\n");
        ...
        case MSG_RUN_EXERCISE:
            if (verbose)
                RtPrintf("Received MESSAGE_RUN_EXERCISE from GUI process.\n");
        ...
        case MSG_PAUSE_EXERCISE:
            if (verbose)
                RtPrintf("Received MESSAGE_PAUSE_EXERCISE from GUI process.\n");
        ...
        case MSG_RESET_HOST:
            if (verbose)
                RtPrintf("Received MESSAGE_RESET_HOST from GUI process.\n");
        ...
        case MSG_RESET_IG:
            if (verbose)
                RtPrintf("Received MESSAGE_RESET_IG from GUI process.\n");
        ...
        case MSG_SHUTDOWN:
            if (verbose)
                RtPrintf("Received MESSAGE_SHUTDOWN from GUI process.\n");
        ...
        case MSG_SET_ADDR:
            if (verbose)
                RtPrintf("Received MESSAGE_SET_ADDR from GUI process.\n");
        ...
        case MSG_ADD_HAT:
            if (verbose)
                RtPrintf("Received MESSAGE_ADD_HAT from GUI process.\n");
        ...
        case MSG_ADD_LOS:
            if (verbose)
                RtPrintf("Received MESSAGE_ADD_LOS from GUI process.\n");
        ...
        case MSG_REMOVE_HAT:
            if (verbose)
                RtPrintf("Received MESSAGE_REMOVE_HAT from GUI process.\n");
        ...
        case MSG_REMOVE_LOS:
            if (verbose)
                RtPrintf("Received MESSAGE_REMOVE_LOS from GUI process.\n");
        ...
        case MSG_BEGIN_RECORD:
            if (verbose)
                RtPrintf("Received MESSAGE_BEGIN_RECORD from GUI process.\n");
        ...
        case MSG_END_RECORD:
            if (verbose)
                RtPrintf("Received MESSAGE_END_RECORD from GUI process.\n");
        ...
        case MSG_BEGIN_PLAYBACK:
            if (verbose)
                RtPrintf("Received MESSAGE_BEGIN_PLAYBACK from GUI process.\n");
        ...
        case MSG_END_PLAYBACK:
            if (verbose)
                RtPrintf("Received MESSAGE_END_PLAYBACK from GUI process.\n");
        ...
        case MSG_PAUSE_RECPLAYBACK:
            if (verbose)
                RtPrintf("Received MESSAGE_PAUSE_RECPLAYBACK from GUI process.\n");
        ...
        case MSG_RESUME_RECPLAYBACK:
            if (verbose)
                RtPrintf("Received MESSAGE_RESUME_RECPLAYBACK from GUI process.\n");
        ...
        case MSG_SET_DATABASE:
            ...
            if (verbose)
                RtPrintf("Received MESSAGE_SET_DATABASE from GUI process.  "
                         "Number = %d\n", igc.db_number);
        ...
        case MSG_SET_TRACKER_STATE:
            if (verbose)
                RtPrintf("Received MESSAGE_SET_TRACKER_STATE from GUI process.\n");
        ...
        case MSG_SET_TRACKER_BORESIGHT:
            if (verbose)
                RtPrintf("Received MESSAGE_SET_TRACKER_BORESIGHT from GUI process.\n");
        ...
        case MSG_SET_IG_MODE:
            ...
            if (verbose)
                RtPrintf("Received MESSAGE_SET_IG_MODE from GUI process.  Mode = %d\n",
                         igc.ig_mode);
        ...
        case MSG_SET_IG_CONTROL:
            ...
            if (verbose)
                RtPrintf("Received MSG_SET_IG_CONTROL from GUI process.  Smoothing = %s, Substitution = %s\n",
        ...
        case MSG_SET_HUD_STATE:
            ...
            if (verbose)
                RtPrintf("Received MESSAGE_SET_HUD_STATE from GUI process.  State = %d\n",
                         (hud_enable ? 1 : 0));
        ...
        case MSG_SYNC_UPDATE_ENTITIES_VIEW: {
            if (verbose)
                RtPrintf("Received MSG_SYNC_UPDATE_ENTITIES_VIEW from GUI process.\n");
        ...
        case MSG_SYNC_UPDATE_VIEWS_VIEW: {
            if (verbose)
                RtPrintf("Received MSG_SYNC_UPDATE_VIEWS_VIEW from GUI process.\n");
        ...
        case MSG_SYNC_UPDATE_ENV_VIEW: {
            if (verbose)
                RtPrintf("Received MSG_SYNC_UPDATE_ENV_VIEW from GUI process.\n");
        ...
        case MSG_SET_CIGI_OPTIONS: {
            if (verbose)
                RtPrintf("Received MSG_SET_BYTE_ORDER from GUI process.\n");
...
void BuildViewPacketsWorker(VIEW *view)
{
    ...
    if (view->noncigi.def_has_changed) {
        if (verbose)
            RtPrintf("View definition has changed\n");
    ...
    if (view->noncigi.ctrl_has_changed) {
        if (verbose)
            RtPrintf("View control has changed\n");
...
void BuildViewGroupPacketsWorker(VIEWGROUP *group)
{
    ...
    if (group->noncigi.cigi_viewgroup_has_changed) {
        if (verbose)
            RtPrintf("View group has changed\n");
...
void AddPacketsFromQueue(RTXSharedBufferQ *queue)
{
    ...
    do {
        if (size = queue->Pop((char *)buffer, CIGI_MAX_PACKET_SIZE)) {
            ...
            case CIGI_START_OF_FRAME_OPCODE:
                ...
                // in a script file.
                if (verbose)
                    RtPrintf("AddPacketsFromQueue():  Encountered a FRAME script command.\n");

                // Set the event so the driver knows when it's okay to continue
                // parsing the script file.
                RtSetEvent(g_FrameWaitEventHdl);
...
long DoHatHotResponse(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("HAT Response:\n");
        RtPrintf("  ID = %d\n", msg->hat_hot_id);
        RtPrintf("  Response Type = %d\n", msg->response_type);
        RtPrintf("  Valid = %d\n", msg->valid);
        RtPrintf("  Height = %lf\n", msg->height);
        RtPrintf("\n");
    }
...
long DoHatHotExtResponse(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("HAT Extended Response:\n");
        RtPrintf("  ID = %d\n", msg->hat_hot_id);
        RtPrintf("  Valid = %d\n", msg->valid);
        RtPrintf("  HAT = %lf\n", msg->hat);
        RtPrintf("  HOT = %lf\n", msg->hot);
        RtPrintf("  Normal Azimuth = %f\n", msg->normal_azimuth);
        RtPrintf("  Normal Elevation = %f\n", msg->normal_elevation);
        RtPrintf("  Material Code = %u\n", msg->material_code);
        RtPrintf("\n");
    }
...
long DoLOSExtResponse(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("LOS Extended Response:\n");
        RtPrintf("  ID = %d\n", msg->los_id);
        RtPrintf("  Valid = %d\n", msg->valid);
        RtPrintf("  Visible = %d\n", msg->visible);
        RtPrintf("  Entity ID = %d\n", msg->entity_id);
        RtPrintf("  Entity ID Valid = %d\n", msg->entity_id_valid);
        RtPrintf("  Latitude/X = %lf\n", msg->lat_x);
        RtPrintf("  Longitude/Y = %lf\n", msg->lon_y);
        RtPrintf("  Altitude/Z = %lf\n", msg->alt_z);
        RtPrintf("  Range = %lf\n", msg->range);
        RtPrintf("  Range Valid = %d\n", msg->range_valid);
        RtPrintf("  Normal Azimuth = %f\n", msg->normal_azimuth);
        RtPrintf("  Normal Elevation = %f\n", msg->normal_elevation);
        RtPrintf("  Material Code = %u\n", msg->material_code);
        RtPrintf("  RGBA = %d %d %d %d\n", msg->red, msg->green, msg->blue, msg->alpha);
        RtPrintf("  Response Count = %d\n", msg->response_count);
        RtPrintf("\n");
    }
...
long DoSensorResponse(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("Sensor Response:\n");
        RtPrintf("  View ID = %d\n", msg->view_id);
        RtPrintf("  Sensor ID = %d\n", msg->sensor_id);
...
long DoSensorExtResponse(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("Sensor Extended Response:\n");
        RtPrintf("  View ID = %d\n", msg->view_id);
        RtPrintf("  Sensor ID = %d\n", msg->sensor_id);
        RtPrintf("  Sensor Status = %d\n", msg->sensor_status);
        RtPrintf("  Entity ID = %d\n", msg->entity_id);
...
long DoPositionResponse(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("Position Response:\n");
        RtPrintf("  Object Class = %d\n", msg->object_class);
        RtPrintf("  Object ID = %d\n", msg->object_id);
        RtPrintf("  Articulated Part ID = %d\n", msg->artpart_id);
        RtPrintf("  Coord System = %d\n", msg->coord_system);
        RtPrintf("  Latitude/X = %lf\n", msg->lat_x);
        RtPrintf("  Longitude/Y = %lf\n", msg->lon_y);
        RtPrintf("  Altitude/Z = %lf\n", msg->alt_z);
        RtPrintf("  Roll = %f\n", msg->roll);
...
long DoWeatherResponse(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("Weather Response:\n");
        RtPrintf("  Request ID = %d\n", msg->request_id);
        RtPrintf("  Humidity = %d\n", msg->humidity);
        RtPrintf("  Air Temperature = %f\n", msg->air_temp);
        RtPrintf("  Visibility = %f\n", msg->visibility);
        RtPrintf("  Wind Speed (horizontal) = %f\n", msg->windspeed_horz);
        RtPrintf("  Wind Speed (vertical) = %f\n", msg->windspeed_vert);
        RtPrintf("  Wind Direction = %f\n", msg->wind_direction);
        RtPrintf("  Barometric Pressure = %f\n", msg->baro_pressure);
    }
...
long DoAerosolResponse(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("Aerosol Response:\n");
        RtPrintf("  Request ID = %d\n", msg->request_id);
        RtPrintf("  Layer ID = %d\n", msg->layer_id);
        RtPrintf("  Concentration = %f\n", msg->concentration);
        RtPrintf("\n");
    }
...
long DoMaritimeResponse(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("Maritime Surface Conditions Response:\n");
        RtPrintf("  Request ID = %d\n", msg->request_id);
        RtPrintf("  Surface Height = %f\n", msg->surface_height);
        RtPrintf("  Surface Temperature = %f\n", msg->surface_temp);
        RtPrintf("  Surface Clarity = %f\n", msg->surface_clarity);
        RtPrintf("\n");
    }
...
long DoTerrestrialResponse(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("Terrestrial Surface Conditions Response:\n");
        RtPrintf("  Request ID = %d\n", msg->request_id);
        RtPrintf("  Condition ID = %d\n", msg->condition_id);
        RtPrintf("\n");
    }
...
long DoCollisionSegNotification(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("Collision Detection Segment Notification:\n");
        RtPrintf("  Entity ID = %d\n", msg->entity_id);
        RtPrintf("  Segment ID = %d\n", msg->segment_id);
...
long DoCollisionVolNotification(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("Collision Detection Volume Notification:\n");
        RtPrintf("  Entity ID = %d\n", msg->entity_id);
        RtPrintf("  Volume ID = %d\n", msg->volume_id);
        RtPrintf("  Collision Type = %d\n", msg->collision_type);
        RtPrintf("  Contacted Entity ID = %d\n", msg->contacted_entity_id);
        RtPrintf("  Contacted Volume ID = %d\n", msg->contacted_volume_id);
        RtPrintf("\n");
    }
...
long DoAnimationStopNotification(const int sessionid, void *packet)
{
    ...
    if (verbose) {
        RtPrintf("Animation Stop Notification:\n");
        RtPrintf("  Entity ID = %d\n", msg->entity_id);
        RtPrintf("\n");
    }
